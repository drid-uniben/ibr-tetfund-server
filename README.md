...
1.  **Clone the repository:**
    ```bash
    git clone https://github.com/unibeninterns/ibr-tetfund-server.git
    cd ibr-tetfund-server
    ```
...
This project uses Agenda to run the AI review generation in the background. The `pnpm worker` command starts a process that listens for and processes these jobs from the database queue. It is necessary to run this worker alongside the main application for AI reviews to be generated automatically.
